...
AND UPPER(A.EMAIL_ADDR) LIKE '%GMAIL.COM%'
AND UPPER(B.EMAIL_ADDR) NOT LIKE '%GMAIL.COM%';
...
--aqt export,file=Email_updates_GMAIL_email.xlsx,type=excel,sheet=EMAILS,filemode=replace,prompt=no,header=yes,nulls=blank,sdelim=dquote
...
def build_atGMAIL_list_query(GMAIL_SQL):
    ...
    with open (GMAIL_SQL, "w") as file:
        ...
        wb = openpyxl.load_workbook(filename='Email_updates_GMAIL_email.xlsx', read_only=False)
        sheet = wb.get_sheet_by_name('EMAILS')
        for row in range(2, sheet.max_row +1 ):
            EMPL_ID = sheet['A' + str(row)].value
            file.write("'" + EMPL_ID + "', ")
...
def build_atGMAIL_insert(GMAIL):
    
    with open (GMAIL, "w") as file:
        wb = openpyxl.load_workbook(filename='Email_updates_GMAIL_email.xlsx', read_only=False)
        sheet = wb.get_sheet_by_name('EMAILS')
        for row in range(2, sheet.max_row +1 ):
            EMPL_ID = sheet['A' + str(row)].value
...
def rename_files(BUSN, PERS, GMAIL_SQL, UFL):
    ...
    print(" . ")
    os.rename(r'UFL_EMAIL_SYNC_SQL.txt',r'GMAIL_EMAIL_SYNC_SQL_' + str(Current_Date) + "_" + "SR" + SR_Number + '.txt')
    print(" . ")
    os.rename(r'UFL_EMAIL_INSERTS.txt',r'GMAIL_EMAIL_INSERTS_' + str(Current_Date) + "_" + "SR" + SR_Number + '.txt')
    print("Done")
...
def main():
    ...
    UFL_SQL = "GMAIL_EMAIL_SYNC_SQL.txt"
    UFL = "GMAIL_EMAIL_INSERTS.txt"
    build_BUSN_queries(BUSN)
    build_PERS_queries(PERS)
    build_atGMAIL_list_query(GMAIL_SQL)
    build_atGMAIL_insert(GMAIL)
    rename_files(BUSN, PERS, GMAIL_SQL, UFL)
